            ...
            <div class="card shadow">
                <div class="card-header border-0" >
                    ...
                    <div class="row align-items-center">
                        <div class="col-md-2">
                            <h3 class="mb-0">Reporte Ingresos por implantes</h3>
                        </div>
                    </div>
                                        ...
                                        <table id="tablaDetalleIngresos" class="table align-items-center table-flush">
                                            <thead class="thead-light">
                                                <tr>
                                                    ...
                                                    <th scope="col">Costo implante</th>
                                            ...
                                            @foreach($pagos as $pago)
                                                <tr>
                                                    ...
                                                    <td>{{$pago->implante_costo}}</td>
                                                    <td>{{$pago->monto_acumulado}}</td>
                                                    <td>{{$pago->implante_costo-$pago->monto_acumulado}}</td>
                                                    <td>{{'implante('.$pago->implante_id.')-'.$pago->tipo_implante_nombre.'-'.$pago->implante_marca.'-'.$pago->implante_medida.'-pieza#'.$pago->pieza_dental}}</td>
...
//datatables 
    ...
    quitarFiltro.addEventListener('click', function() {
        window.location.href = '{{ route('reporte.ingresosImplantes') }}';
    });
    ...
    function actualizarGraficas(){
        ...
        fetch("{{ route('reporte.ingresosImplantes2') }}", {
                    method: 'POST',
                    headers: {
                ...
                .then(data => {
                    ...
                    data.pagos.forEach(function(pago) {
                        ...
                        var montoAcumulado = pago.implante_costo - pago.monto_acumulado;
                        var newRow = '<tr>' +
